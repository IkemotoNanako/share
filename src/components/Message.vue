    ...
    <div>
      ...
      <div class="message">
        <div class="flex">
          ...
          <img class="icon detail" src="../assets/detail.png" @click="$router.push({path:'/detail/'+ value.item.id,params:{id:value.item.id}})" alt v-if="profile"/>
        </div>
...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach(element =>{
          if (element.user_id == this.$store.state.user.id) {
            axios
            .delete("https://limitless-crag-44462.herokuapp.com/like", {
              data: {
            ...
            axios
            .post("https://limitless-crag-44462.herokuapp.com/like", {
              shares_id:this.shares[index].item.id,
              user_id:this.$store.state.user.id
              })
              .then(response => {
    ...
    del(index) {
      axios
      .delete(
        "https://limitless-crag-44462.herokuapp.com/shares/" + this.shares[index].item.id)
      .then(response => {
        console.log(response);
        });
       },
       async getShares() {
          ...
          let shares = await axios.get(
            "https://limitless-crag-44462.herokuapp.com/shares"
       );
